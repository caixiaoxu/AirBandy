...
object TextureUtil {
    ...
    fun loadTexture(res: Resources, rId: Int): Int {
        ...
        if (0 == textureIds[0]) {
            LogUtil.logE("Could not generate a new OpenGL texture object.")
            return 0
        }
        //加载无压缩原始数据图片
        ...
        if (null == bitmap) {
            LogUtil.logE("Resourece ID $rId could not be decoded.")
    ...
    fun loadCubeMap(res: Resources, cubeIds: IntArray): Int {
        ...
        for (i in cubeIds.indices) {
            ...
            if (null == cubeBitmaps[i]) {
                LogUtil.logE("Resource ID ${cubeIds[i]} could not be decoded.")
